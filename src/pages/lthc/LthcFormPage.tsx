...
export default function LthcFormPage() {
    ...
    console.log("ค่าของ isReduceSaModalOpen ตอนนี้คือ:", isReduceSaModalOpen);
    ...
    useEffect(() => {
        if (policyOriginMode === 'newPolicy') {
            if (lifeReadyMode === 'package') {
                setSelectedHealthPlans(prev => ({ ...prev, lifeReadySA: PACKAGE_LR_SA, lifeReadyPPT: DEFAULT_LR_PPT_PACKAGE }));
            } else {
                setSelectedHealthPlans(prev => ({ ...prev, lifeReadySA: Math.max(prev.lifeReadySA, MIN_LR_SA_NORMAL_MODE), lifeReadyPPT: prev.lifeReadyPPT === DEFAULT_LR_PPT_PACKAGE ? DEFAULT_LR_PPT_NORMAL : prev.lifeReadyPPT }));
            }
    ...
    const handleLifeReadySAInput = (inputValue: string) => { let value = parseInt(inputValue, 10); if (isNaN(value)) value = 0; setSelectedHealthPlans(prev => ({ ...prev, lifeReadySA: Math.max(0, value) })); };
    const handleLifeReadyPPTChange = (value: LifeReadyPaymentTerm) => { setSelectedHealthPlans(prev => ({ ...prev, lifeReadyPPT: value })); };
    const handleLifeReadySAInputBlur = (e: React.FocusEvent<HTMLInputElement>) => { if (policyOriginMode === 'newPolicy' && lifeReadyMode === 'normal') { let currentSA = parseInt(e.target.value, 10); if (isNaN(currentSA) || (currentSA > 0 && currentSA < MIN_LR_SA_NORMAL_MODE)) { setSelectedHealthPlans(prev => ({ ...prev, lifeReadySA: MIN_LR_SA_NORMAL_MODE })); } else if (currentSA <= 0 && e.target.value !== "") { setSelectedHealthPlans(prev => ({ ...prev, lifeReadySA: MIN_LR_SA_NORMAL_MODE })); } } };
    ...
    const handleIHUCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => { const isChecked = e.target.checked; setIsIHUSelected(isChecked); setSelectedHealthPlans(prev => ({ ...prev, iHealthyUltraPlan: isChecked ? (prev.iHealthyUltraPlan !== null ? prev.iHealthyUltraPlan : 'Bronze') : null })); };
    ...
    const handleMEBCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => { const isChecked = e.target.checked; setIsMEBSelected(isChecked); setSelectedHealthPlans(prev => ({ ...prev, mebPlan: isChecked ? (prev.mebPlan !== null ? prev.mebPlan : 1000) : null })); };
        ...
        <div className="space-y-6 pb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 items-start">
                    <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-x-4 items-end">
                            <div>
                                ...
                                <select id="currentEntryAge" value={policyholderEntryAge}
                                    onChange={(e) => setPolicyholderEntryAge(Math.max(1, Math.min(80, parseInt(e.target.value, 10) || 1)))}
                                    className="mt-1 p-2 w-full border rounded-md shadow-sm text-sm" style={{maxWidth: '100px'}} >
                    ...
                    <div className="space-y-3 flex flex-col items-end">
                        ...
                        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 w-full sm:w-auto justify-end">
                            <button onClick={() => setPolicyOriginMode('newPolicy')}
                            ...
                            <button onClick={() => setPolicyOriginMode('existingPolicy')}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <section className={`p-4 border rounded-lg shadow-md space-y-4 ${policyOriginMode === 'existingPolicy' ? 'bg-orange-50' : (lifeReadyMode === 'package' ? 'bg-green-50' : 'bg-blue-50')}`}>
                    ...
                    {policyOriginMode === 'existingPolicy' && (
                        <div className="animate-fadeIn mb-4 w-full sm:w-auto">
                            <label htmlFor="existingPolicyEntryAge" className="block text-sm font-medium text-gray-700">อายุที่เริ่มทำ LifeReady เดิม:</label>
                            <select id="existingPolicyEntryAge" value={existingPolicyEntryAge || ''}
                                ...
                                {existingAgeOptions.map(age => (<option key={`existing-${age}`} value={age}>{age}</option>))}
                            </select>
                            {existingPolicyEntryAge !== undefined && policyholderEntryAge > existingPolicyEntryAge && (
                                <p className="text-xs text-gray-600 mt-1">(จ่ายเบี้ย LifeReady เดิมมาแล้ว: {policyholderEntryAge - existingPolicyEntryAge} ปี)</p>
                            )}
                            <hr className="my-4 border-orange-200"/>
                        </div>
                    )}
                    <div className="flex space-x-2 mb-4">
                       <button onClick={() => setLifeReadyMode('normal')}
                        ...
                        <button onClick={() => setLifeReadyMode('package')}
                    ...
                    <div className="space-y-3">
                        <div>
                            ...
                            <input id="lrSA-form" type="number" value={displayLifeReadySA} onChange={(e) => handleLifeReadySAInput(e.target.value)}
                                disabled={lifeReadyMode === 'package'} onBlur={handleLifeReadySAInputBlur}
                                className="mt-1 p-2 w-full border rounded-md shadow-sm disabled:bg-gray-200" step="50000" />
                        </div>
                        <div>
                            <label htmlFor="lrPPT-form" className="block text-sm font-medium text-gray-700">ระยะเวลาชำระเบี้ย:</label>
                            <select id="lrPPT-form" value={displayLifeReadyPPT}
                                onChange={(e) => handleLifeReadyPPTChange(Number(e.target.value) as LifeReadyPaymentTerm)}
                                disabled={lifeReadyMode === 'package'} className="mt-1 p-2 w-full border rounded-md shadow-sm disabled:bg-gray-200" >
                    ...
                    <div className="p-3 border rounded-md bg-indigo-50">
                        ...
                        {isIHUSelected && (
                            <div className="pl-6 space-y-1 animate-fadeIn">
                                ...
                                <select id="ihuPlan-form" value={selectedHealthPlans.iHealthyUltraPlan === null ? 'Smart' : selectedHealthPlans.iHealthyUltraPlan}
                                    onChange={(e) => handleIHUPlanChange(e.target.value as IHealthyUltraPlan)}
                                    className={`p-1.5 border rounded-md text-xs w-full ${selectedHealthPlans.iHealthyUltraPlan && iHealthyUltraPlanColors[selectedHealthPlans.iHealthyUltraPlan as string] ? iHealthyUltraPlanColors[selectedHealthPlans.iHealthyUltraPlan as string] : 'text-gray-700'}`} >
                                    {(['Smart', 'Bronze', 'Silver', 'Gold', 'Diamond', 'Platinum'] as IHealthyUltraPlan[]).map(planName => (
                                        <option key={planName} value={planName} className={iHealthyUltraPlanColors[planName] ?? ''}>{planName}</option>
                                    ))}
                                </select>
                                <div className="pt-1">
                                    <p className="text-xs text-gray-500">เบี้ยประกัน iHealthy Ultra ปีแรก:</p>
                                    <p className={`text-sm font-semibold ${selectedHealthPlans.iHealthyUltraPlan && iHealthyUltraPlanColors[selectedHealthPlans.iHealthyUltraPlan as string] ? iHealthyUltraPlanColors[selectedHealthPlans.iHealthyUltraPlan as string] : 'text-indigo-700'}`}>
                    ...
                    <div className="p-3 border rounded-md bg-lime-50">
                        ...
                        {isMEBSelected && (
                            <div className="pl-6 space-y-1 animate-fadeIn">
                                ...
                                <select id="mebPlanValue-form"
                                    value={selectedHealthPlans.mebPlan === null ? 500 : selectedHealthPlans.mebPlan}
                                    ...
                                    {( [500, 1000, 2000, 3000, 4000, 5000] as MEBPlan[]).map(planValue => (
                                        <option key={planValue} value={planValue}>{planValue.toLocaleString()}</option>
                                    ))}
                                </select>
                                <div className="pt-1">
                ...
                <section className={`p-4 border rounded-lg shadow-md space-y-4 ${iWealthyMode === 'manual' ? 'bg-emerald-50' : 'bg-sky-50'}`}>
                    ...
                    {/* ================== START CHANGE: UI ใหม่สำหรับการลดทุน ================== */}
                    <div className="space-y-2">
                        ...
                        <div className="flex rounded-md shadow-sm">
                            <button
                                ...
                                className={`flex-1 px-4 py-2 text-sm font-medium rounded-l-md border border-gray-300 ${saReductionStrategy.type === 'auto' ? 'bg-sky-600 text-white z-10' : 'bg-white hover:bg-gray-50'}`}
                            >
                                อัตโนมัติ (แนะนำ)
                            </button>
                            <button
                                type="button"
                                onClick={() => {
                                    // ถ้าปัจจุบันเป็น auto ให้เปลี่ยนเป็น manual list ว่าง
                                    if (saReductionStrategy.type === 'auto') {
                                        setSaReductionStrategy({ type: 'manual', ages: [] });
                                    }
                                    // เปิด Modal
                                    setIsReduceSaModalOpen(true);
                                }}
                                className={`flex-1 px-4 py-2 text-sm font-medium rounded-r-md border border-l-0 border-gray-300 ${saReductionStrategy.type === 'manual' ? 'bg-emerald-600 text-white z-10' : 'bg-white hover:bg-gray-50'}`}
                            >
                                กำหนดเอง...
                            </button>
                        </div>
                        {saReductionStrategy.type === 'manual' && (
                            <p className="text-xs text-right pr-1 text-gray-600">
                                ช่วงอายุที่ลดทุน {saReductionStrategy.ages.length} ช่วง (คลิกเพื่อแก้ไข)
                            </p>
                        )}
                    </div>
                    {/* ================== END CHANGE ================== */}
            ...
            <section className="mt-6 p-6 border rounded-lg shadow-xl bg-gradient-to-r from-sky-500 to-indigo-600">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <button onClick={runCalculation} disabled={isLoading || showIHUWarning}
                ...
                {showIHUWarning && (
                    <div className="mt-3 text-sm text-yellow-300 flex items-center justify-center md:justify-start">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.332-.166 3.001-1.742 3.001H4.42c-1.576 0-2.492-1.67-1.742-3.001l5.58-9.92zM10 14a1 1 0 100-2 1 1 0 000 2zm0-7a1 1 0 00-1 1v3a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
